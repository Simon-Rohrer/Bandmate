...
const Bands = {
    ...
    async showBandDetails(bandId) {
        ...
        if (bandRiderBtn) {
            bandRiderBtn.style.display = canManage ? 'inline-flex' : 'none';
            bandRiderBtn.onclick = async () => await this.openBandRiderEditor(bandId);
        }
        if (deleteBandBtn) {
    ...
    async openBandRiderEditor(bandId) {
        this.currentBandId = bandId;
        const band = await Storage.getBand(bandId);
        let members = await Storage.getBandMembers(bandId);

        if (!band || !members || members.length === 0) {
            UI.showToast('Band oder Mitglieder nicht gefunden', 'error');
            return;
        }

        members = await this.enrichBandRiderMembers(members);

        // Lade bestehenden Rider
        const existingRider = await Storage.getBandRider(bandId);
        const riderData = existingRider ? existingRider.members || {} : {};

        // Setup PDF Editor Fields
        const pdfFileName = document.getElementById('bandRiderPdfFileName');
        const pdfFontScale = document.getElementById('bandRiderPdfFontScale');
        if (pdfFileName) {
            pdfFileName.value = `Rider ${band.name || 'der Band'}`;
        }
        if (pdfFontScale) {
            pdfFontScale.value = '100';
            this.updateBandRiderFontScaleValue(100);
        }

        // Rendere die Member-Karten
        await this.renderBandRiderMembers(bandId, members, riderData);

        // Öffne das Modal
        UI.openModal('bandRiderModal');

        // Aktualisiere die Vorschau
        await this.refreshBandRiderPreview(bandId, members, riderData);

        // Setup Event Listeners
        this.setupBandRiderEventListeners(bandId, members);
    },

    async renderBandRiderMembers(bandId, members, riderData = {}) {
        ...
        for (const member of members) {
            const memberData = riderData[member.userId] || {};
            const memberCard = document.createElement('div');
            memberCard.className = 'bandrider-member-card';
            memberCard.dataset.userId = member.userId;

            const initials = this.getMemberInitials(member);
            const instrumentName = memberData.instrument || member.defaultInstrument || '';

            memberCard.innerHTML = `
                <div class="bandrider-member-name">
                    <div class="bandrider-member-name-icon">${this.escapeHtml(initials)}</div>
                    <span>${this.escapeHtml(member.displayName || 'Unbekannt')}</span>
                </div>

                <div class="bandrider-member-fields">
                    <div class="bandrider-field-group">
                        <label class="bandrider-field-label">Instrument</label>
                        <input type="text" class="bandrider-field-input rider-instrument"
                               value="${this.escapeHtml(instrumentName)}"
                               placeholder="z.B. Keyboard, Gitarre">
                    </div>

                    <div class="bandrider-field-group">
                        <label class="bandrider-field-label">XLR / Audio-Eingänge</label>
                        <input type="text" class="bandrider-field-input rider-audio-inputs"
                               value="${this.escapeHtml(memberData.audioInputs || '')}"
                               placeholder="z.B. 2x XLR Stereo, 1x 3.5mm Klinke">
                    </div>

                    <div class="bandrider-field-group">
                        <label class="bandrider-field-label">Monitoring</label>
                        <select class="bandrider-field-select rider-monitoring">
                            <option value="">-- Bitte auswählen --</option>
                            <option value="inears" ${memberData.monitoring === 'inears' ? 'selected' : ''}>In-Ears</option>
                            <option value="floor" ${memberData.monitoring === 'floor' ? 'selected' : ''}>Floor Monitor</option>
                            <option value="both" ${memberData.monitoring === 'both' ? 'selected' : ''}>In-Ears + Monitor</option>
                            <option value="none" ${memberData.monitoring === 'none' ? 'selected' : ''}>Kein Monitoring</option>
                        </select>
                    </div>

                    <div class="bandrider-field-group">
                        <label class="bandrider-field-label">Zusätzliche Anforderungen</label>
                        <textarea class="bandrider-field-textarea rider-notes"
                               placeholder="z.B. Spezielle Anforderungen, Lautstärkepräferenzen...">${this.escapeHtml(memberData.notes || '')}</textarea>
                    </div>
                </div>
            `;
    ...
    setupBandRiderEventListeners(bandId, members) {
        const container = document.getElementById('bandRiderMembersContainer');
        if (!container) return;

        // Live update preview on input changes
        container.oninput = async () => {
            await this.refreshBandRiderPreview(bandId, members);
        };
        ...
        const pdfFileName = document.getElementById('bandRiderPdfFileName');
        if (pdfFileName) {
            pdfFileName.oninput = async () => {
                await this.refreshBandRiderPreview(bandId, members);
            };
        }

        // PDF fontScale slider
        const pdfFontScale = document.getElementById('bandRiderPdfFontScale');
        if (pdfFontScale) {
            pdfFontScale.oninput = async (e) => {
                this.updateBandRiderFontScaleValue(e.target.value);
                await this.refreshBandRiderPreview(bandId, members);
            };
        }
        ...
        if (saveBtn) {
            saveBtn.onclick = async () => await this.saveBandRider(bandId, members);
        }

        // PDF export button
        const pdfBtn = document.getElementById('bandRiderExportPdf');
        if (pdfBtn) {
            pdfBtn.onclick = async () => await this.exportBandRiderPdf(bandId, members);
        }

        // Close button
        const closeBtn = document.getElementById('bandRiderCancel');
        if (closeBtn) {
            closeBtn.onclick = () => UI.closeModal('bandRiderModal');
        }
    },

    updateBandRiderFontScaleValue(value) {
        const display = document.getElementById('bandRiderPdfFontScaleValue');
        if (display) {
            display.textContent = `${Math.round(value)}%`;
        }
    },

    async refreshBandRiderPreview(bandId, members) {
        const riderData = this.collectBandRiderFormData(members);
        const previewFrame = document.getElementById('bandRiderPreviewFrame');
        const loadingDiv = document.getElementById('bandRiderPreviewLoading');
        const fontScaleInput = document.getElementById('bandRiderPdfFontScale');
        const fontScale = fontScaleInput ? parseInt(fontScaleInput.value) / 100 : 1;
        ...
        try {
            ...
            const band = await Storage.getBand(bandId);
            const previewMembers = this.getBandRiderPreviewMembers(members, riderData);
            ...
            previewFrame.srcdoc = this.buildBandRiderPreviewDocument(pages);
        } catch (error) {
            console.error('[Bands] Error refreshing preview:', error);
        } finally {
            if (loadingDiv) {
    ...
    collectBandRiderFormData(members) {
        const riderData = {};
        const container = document.getElementById('bandRiderMembersContainer');

        if (!container) return riderData;

        container.querySelectorAll('.bandrider-member-card').forEach(card => {
            const userId = card.dataset.userId;
            riderData[userId] = {
                instrument: card.querySelector('.rider-instrument')?.value || '',
                audioInputs: card.querySelector('.rider-audio-inputs')?.value || '',
                monitoring: card.querySelector('.rider-monitoring')?.value || '',
                notes: card.querySelector('.rider-notes')?.value || ''
            };
        });
    ...
    async enrichBandRiderMembers(members = []) {
        const safeMembers = Array.isArray(members) ? members : [];

        return await Promise.all(safeMembers.map(async (member) => {
            let user = null;
            try {
                user = member?.userId ? await Storage.getById('users', member.userId) : null;
            } catch (error) {
                console.warn('[Bands] Rider member profile lookup failed:', error);
            }

            const displayName = user
                ? UI.getUserDisplayName(user)
                : (member.displayName || member.username || member.email || 'Unbekannt');

            const instrumentLabels = this.getInstrumentLabels(user?.instrument || member.instrument || '');

            return {
                ...member,
                profile: user,
                displayName,
                defaultInstrument: instrumentLabels.join(', ') || ''
            };
        }));
    },

    getBandRiderMonitoringLabel(value = '') {
        const map = {
            inears: 'In-Ears',
            floor: 'Floor Monitor',
            both: 'In-Ears + Monitor',
            none: 'Kein Monitoring'
        };

        return map[String(value || '').trim()] || String(value || '').trim() || '-';
    },

    getBandRiderPreviewMembers(members = [], riderData = {}) {
        const safeMembers = Array.isArray(members) ? members : [];

        return safeMembers.map((member) => {
            const data = riderData[member.userId] || {};
            return {
                id: member.userId,
                name: member.displayName || 'Unbekannt',
                instrument: data.instrument || member.defaultInstrument || '-',
                mic: data.audioInputs || data.mic || '-',
                monitor: this.getBandRiderMonitoringLabel(data.monitoring || data.monitor),
                extra: data.notes || data.extra || ''
            };
        });
    },

    buildBandRiderPreviewDocument(pages = []) {
        const pageMarkup = (Array.isArray(pages) ? pages : []).map((page) => `
            <div class="bandrider-preview-page">
                ${page}
            </div>
        `).join('');
        ...
        return `
            ...
            <html lang="de">
                <head>
                    ...
                    <style>
                        * { box-sizing: border-box; }
                        ...
                        .bandrider-preview-shell {
                            display: flex;
                            flex-direction: column;
                            align-items: center;
                            gap: 24px;
                        }
                        .bandrider-preview-page {
                            width: 794px;
                            max-width: 100%;
                            background: #ffffff;
                            border-radius: 4px;
                ...
                <body>
                    <div class="bandrider-preview-shell">
                        ${pageMarkup || '<div class="bandrider-preview-page"></div>'}
                    </div>
                </body>
            </html>
        `;
    ...
    async saveBandRider(bandId, members) {
        try {
            const riderData = this.collectBandRiderFormData(members);
            await Storage.createOrUpdateBandRider(bandId, { members: riderData });
            UI.showToast('Bandrider gespeichert!', 'success');
        } catch (error) {
            console.error('[Bands] Error saving rider:', error);
            UI.showToast('Fehler beim Speichern des Riders', 'error');
        }
    },

    async exportBandRiderPdf(bandId, members) {
        try {
            const band = await Storage.getBand(bandId);
            const riderData = this.collectBandRiderFormData(members);
            const fontScaleInput = document.getElementById('bandRiderPdfFontScale');
            const fontScale = fontScaleInput ? parseInt(fontScaleInput.value) / 100 : 1;
            const pdfFileName = document.getElementById('bandRiderPdfFileName');
            const filename = pdfFileName?.value ? this.sanitizePdfFilename(pdfFileName.value) : 'Bandrider.pdf';
            const previewMembers = this.getBandRiderPreviewMembers(members, riderData);
            ...
            UI.showToast('PDF exportiert!', 'success');
        } catch (error) {
            console.error('[Bands] Error exporting PDF:', error);
            UI.showToast('Fehler beim PDF-Export', 'error');
        }
    },

    sanitizePdfFilename(name = '', fallback = 'export.pdf') {
    ...
    getMemberInitials(member) {
        ...
        return name.split(' ').map(part => part.charAt(0).toUpperCase()).join('').substring(0, 2) || 'U';
    }
};
